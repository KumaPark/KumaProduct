...
/**
 * Created by Kuma on 2018-12-18.
 */

public class DeviceConditionScheduleAdapter extends BaseAdapter {
    ...
    public DeviceConditionScheduleAdapter(Context context , OnItemClickListener listener){
    ...
    @Override
    public ScheduleListDTO getItem(int position) {
    ...
    public void bindData(int tag, ScheduleListDayDTO data, LinearLayout llContents) {
        KumaLog.d(" >>>>>>>>>>>>>>>>> bindData >>>>>>>>>>>>>>>>>>>>>>>>>>> : ");
        if( tag == 0 ) {
...
//                setStateData(mArrTv, data);
    ...
    private void addDateView(LinearLayout llContents) {
        ...
        for (int i = 0; i < mArrScheduleListLowDTO.get(position).getmArrScheduleListDayDTO().size(); i++) {
            ScheduleListDayDTO data = mArrScheduleListLowDTO.get(position).getmArrScheduleListDayDTO().get(i);
